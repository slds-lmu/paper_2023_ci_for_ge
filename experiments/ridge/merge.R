...
reg = makeRegistry(Sys.getenv("FINAL_PATH_RIDGE"), packages = c("batchtools", "data.table"))
batchMap(i = 1, fun = function(i) {
  reg_truth = loadRegistry(Sys.getenv("TRUTH_PATH_RIDGE"), make.default = FALSE)
  reg_proxy = loadRegistry(Sys.getenv("PROXY_PATH_RIDGE"), make.default = FALSE)
  reg_ci    = loadRegistry(Sys.getenv("CI_PATH_RIDGE"),    make.default = FALSE)
  ...
  ci_ids = findDone(reg = reg_ci)[[1]]
  tbl_ci = map_dtr(ci_ids, function(i) {
