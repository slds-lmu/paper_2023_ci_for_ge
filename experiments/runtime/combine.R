...
EXPERIMENT_PATH = Sys.getenv("RESAMPLE_PATH_RUNTIME")
EVAL_PATH = Sys.getenv("RUNTIME_PATH")
...
EVAL_CONFIG = list(
  ...
  # conservative_z
  list("conservative_z_315",     "infer_conservative_z", list(x = "conservative_z_315"),                                list()),
...
batchMap(.args = chunked_args, fun = function(.args) {
  map(.args, function(arg) {
    ids = list(x = arg$x, y = arg$y, z = arg$z)
    if (is.na(arg$y)) ids$y = NULL
    if (is.na(arg$z)) ids$z = NULL

    times = map_dbl(ids, function(id) loadResult(id, reg = EXPERIMENT_REG))
    data.table(
      name = arg$name,
