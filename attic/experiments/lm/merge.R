...
reg = makeRegistry(Sys.getenv("FINAL_PATH_LM"), packages = c("batchtools", "data.table"))
batchMap(i = 1, fun = function(i) {
  reg_truth = loadRegistry(Sys.getenv("TRUTH_PATH_LM"), make.default = FALSE)
  reg_proxy = loadRegistry(Sys.getenv("PROXY_PATH_LM"), make.default = FALSE)
  reg_ci    = loadRegistry(Sys.getenv("CI_PATH_LM"),    make.default = FALSE)
