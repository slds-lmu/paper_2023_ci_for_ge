  ...
  callModule(function(input, output, session) {
    observe({
      if (is.null(input$method) || length(input$method) == 0) {
        # If no options are selected, reset to a default value
        updatePickerInput(session, "method", selected = levels(as.factor(as.data.frame(aggrs_base)$method))[1])
      }
    })
    ...
    output$fallbackplot = renderPlotly({
      ...
      method_dat = as.data.frame(aggrs[which(aggrs$method %in% selected), ])
      g = fallback_plot(data = method_dat, x = input$x, y = input$y, colorval = input$color_fallback, method = input$method)
