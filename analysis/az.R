...
truths = map(SIZES, function(n) {
  ...
  data.table(n = n, truth = mean(truth_ests), se_truth = sd(truth_ests) / sqrt(REPS_TRUTH))
}) |> rbindlist()

dt = merge(var_ests, truths, by = "n")

saveRDS(dt, here("results", "az", "ratio.rds"))
